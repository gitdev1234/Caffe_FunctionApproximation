...
TEST_CASE ("transformations, scaling") {
    ANN ann("");
    ...
    SECTION ("scaling") {
        vector<double> scaledVals   = ann.scaleVector(randVals,65.1234,true);
        vector<double> reScaledVals = ann.scaleVector(scaledVals,65.1234,false);
        for (int i = 0; i < scaledVals.size(); i++) {
            REQUIRE(scaledVals[i] == randVals[i] / 65.1234);
            REQUIRE(reScaledVals[i] == scaledVals[i] * 65.1234);
...
TEST_CASE("Sinus") {
    ...
    SECTION( "vector learning on random weights works" ) {
        ...
        while (d <= 25.0) {
            d += 0.05;
            inputValues.push_back(d);
            expectedResults.push_back(sin(d));
        }
